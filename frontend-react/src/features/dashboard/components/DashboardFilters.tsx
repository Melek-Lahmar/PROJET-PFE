...
export function DashboardFilters({ filters, onChange }: { filters: DashboardFiltersType; onChange: (patch: Partial<DashboardFiltersType>) => void }) {
  ...
  return (
    <div className="pro-filters">
      ...
      <label><span>{t("dashboard.filters.depot")}</span><input inputMode="numeric" placeholder="Tous" value={filters.depotNo ?? ""} onChange={(e) => onChange({ depotNo: e.target.value ? Number(e.target.value) : null })} /></label>
      ...
      <label><span>Top N</span><select value={filters.top ?? 10} onChange={(e) => onChange({ top: Number(e.target.value) })}><option value={5}>5</option><option value={10}>10</option><option value={15}>15</option></select></label>
    </div>
  );
}
