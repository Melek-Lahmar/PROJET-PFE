...
import { getConfirmateurBlList, getConfirmateurDevis, getConfirmateurOrders, getConfirmateurReclamations } from "../../confirmateur/api/confirmateurApi";
...
export function ConfirmateurDashboardPage() {
  ...
  const reclamationsQuery = useQuery({ queryKey: ["confirmateur", "reclamations", "dashboard"], queryFn: () => getConfirmateurReclamations({ tab: "a-traiter" }) });

  const orders = ordersQuery.data ?? [];
  const bl = blQuery.data ?? [];
  const devis = devisQuery.data ?? [];
  const reclamations = reclamationsQuery.data ?? [];
  const totalAmount = orders.reduce((sum, item) => sum + Number(item.dO_TotalTTC ?? 0), 0);
  const isLoading = ordersQuery.isLoading || blQuery.isLoading || devisQuery.isLoading || reclamationsQuery.isLoading;
  ...
  return (
    <div className="w-full space-y-5 pb-10">
      <section className="rounded-2xl border border-border/70 bg-card px-5 py-5 shadow-sm md:px-7">
        <div className="max-w-3xl">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Supervisez vos bons de commande, bons de livraison, devis B2B et réclamations pour assurer une validation fluide.
          </p>
        </div>
      </section>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
        <StatCard label="Total dossiers" value={orders.length + bl.length + devis.length + reclamations.length} hint="BC + BL + Devis + réclamations" />
        ...
        <StatCard label="Réclamations" value={reclamations.length} hint="À prendre en charge" />
      ...
      <section className="grid gap-4 xl:grid-cols-4">
        ...
        <ControlCard title="Réclamations" href="/confirmateur/reclamations" description="Liste dédiée des réclamations client/livreur à prendre en charge et clôturer." />
      </section>
    </div>
  );
}
