...
# ─── Style IDs réels du document (français) ────────────────────────────────
SID = {
    'Normal':          'Normal',
    'Heading 1':       'Titre1',
    'Heading 2':       'Titre2',
    ...
    'List Paragraph':  'Paragraphedeliste',
}
...
def make_para(text, style_name):
    ...
    pStyle.set(qn('w:val'), SID.get(style_name, style_name))
    pPr.append(pStyle)
    p.append(pPr)
    if text:
        r = OxmlElement('w:r')
...
def make_border_el(side, color='808080'):
...
def make_cell(text, bold=False, is_header=False):
    tc = OxmlElement('w:tc')
    ...
    for side in ['top', 'left', 'bottom', 'right']:
        tcBorders.append(make_border_el(side))
    ...
    if is_header:
        ...
        shd.set(qn('w:fill'), '2E4057')   # bleu foncé comme v0
        tcPr.append(shd)
    ...
    # Font size 20 (10pt) for table content
    sz = OxmlElement('w:sz')
    sz.set(qn('w:val'), '20')
...
def make_table(headers, rows):
    ...
    for side in ['top', 'left', 'bottom', 'right', 'insideH', 'insideV']:
        tblBorders.append(make_border_el(side, 'auto'))
...
def ins(anchor, elems):
    """Insère les éléments dans l'ordre avant l'ancre."""
    for e in elems:
        anchor.addprevious(e)

# Raccourcis
def N(t):  return make_para(t, 'Normal')
def H1(t): return make_para(t, 'Heading 1')
def H2(t): return make_para(t, 'Heading 2')
def H3(t): return make_para(t, 'Heading 3')
def H4(t): return make_para(t, 'Heading 4')
def CF(t): return make_para(t, 'Caption Figure')
def CT(t): return make_para(t, 'Caption Tableau')
def LP(t): return make_para(t, 'List Paragraph')
def BR():  return make_para('', 'Normal')
def PB():  return page_break()

# ─── Supprimer les anciens chapitres 6, 7, 8 (body children 567-721) ───────
children = list(body)
to_remove = children[567:722]
...
c6 += [
    H1('Chapitre 6 : Sprint 3 — Administration, confirmateur, réclamations et tableau de bord'),
    H2('1. Introduction'),
...
c6.append(make_table(
    ...
    [
        ...
        ['US4', 'Confirm.', 'Consulter et filtrer les bons de commande', '/api/confirmateur/commandes, /api/confirmateur/bc', 'Melek', '3 j'],
...
c6 += [
    ...
    N("Lorsqu'un client crée une réclamation (POST /api/reclamations), la Web API enregistre le cas dans F_RECLAMATION et diffuse l'événement NouveauCas à tous les confirmateurs connectés via le ReclamationHub SignalR. Le confirmateur disponible prend en charge le cas (POST /reprendre) : son identifiant est associé à la réclamation et l'événement CasAssigne est diffusé. Le confirmateur peut modifier le statut (PUT /status → CLOTUREE ou REFUSEE), envoyer une demande de correction (PUT /correction), créer un bon d'échange (POST /echange) ou réassigner le cas (PUT /assign → événement CasReattribue). Chaque message dans le chat (POST /messages) est diffusé en temps réel via NouveauMessage."),
    N("Mécanisme SignalR — Réclamations : le ReclamationHub maintient deux types de groupes. Le groupe « confirmateurs » reçoit les événements globaux (NouveauCas, CasReattribue). Le groupe « client-{userId} » reçoit les événements propres au client (StatutCasChange, NouveauMessage). Une période de grâce de 5 secondes est appliquée lors d'une déconnexion, afin d'éviter la libération prématurée d'un cas lors d'un simple changement de réseau."),
    ...
    N("Cette interface permet à l'administrateur de consulter, filtrer, créer, modifier et désactiver les produits du catalogue. Elle affiche les informations Sage (code article, désignation, prix) ainsi que les images associées. La synchronisation avec Sage X3 peut être déclenchée manuellement pour mettre à jour les articles, les prix et les stocks."),
    ...
    N("Cette interface permet au confirmateur de déclencher la transformation d'un bon de commande en bon de livraison d'un seul clic. Le système vérifie l'absence de doublon, génère le BL et affiche sa référence immédiatement. En cas d'erreur (stock insuffisant, BC introuvable), un message explicite est présenté. L'interface liste également les bons de livraison déjà générés avec leurs statuts et leurs lignes."),
    ...
    N("Cette interface est le cœur du module de réclamations côté confirmateur. Elle affiche la liste des cas ouverts avec leurs motifs, statuts, sources (CLIENT ou LIVREUR) et confirmateurs assignés. Les nouvelles réclamations apparaissent automatiquement grâce à l'événement SignalR NouveauCas. Le confirmateur peut prendre en charge un cas, modifier son statut (ENVOYEE, EN_COURS_DE_TRAITEMENT, CLOTUREE, REFUSEE), envoyer une demande de correction, consulter les tentatives de livraison associées avec leurs coordonnées GPS, créer un bon d'échange, ajouter une note interne ou réassigner le cas à un autre confirmateur (événement CasReattribue diffusé en temps réel). Le fil de discussion intégré permet d'échanger des messages avec le client en temps réel via NouveauMessage."),
    ...
    N("Cette interface permet à l'administrateur d'administrer les comptes, les profils et les rôles. Elle consomme GET /api/admin/users (avec filtres par rôle) et permet de créer un utilisateur (POST) ou de modifier ses rôles (PUT /api/admin/users/{id}/roles). Les rôles disponibles sont CLIENT, VENDEUR, CONFIRMATEUR, LIVREUR et ADMIN."),
    ...
    N("Le tableau de bord initial présente les indicateurs essentiels permettant de suivre l'activité : nombre de commandes par statut, montants encaissés, réclamations ouvertes, livreurs actifs et taux de livraison. Ces données sont produites par /api/admin/dashboard et /api/dashboard."),
...
c6.append(make_table(
    ...
    [
        ...
        ['Stats confirmateur', 'Vérifier les indicateurs de performance du confirmateur.', 'Cas actifs, clôturés et taux de résolution cohérents.', 'À valider'],
    ],
))
c6 += [
    H2('7. Conclusion'),
    N("Ce sprint a permis de construire le backoffice complet de la solution. L'administrateur dispose de la gestion des produits, des stocks, des dépôts et des utilisateurs. Le confirmateur bénéficie d'un workflow clair pour traiter les bons de commande, déclencher la transformation en bons de livraison et gérer les réclamations en temps réel grâce à SignalR. Ce sprint constitue la charnière entre le parcours client et la phase logistique développée dans le Sprint 4."),
    BR(),
]
...
c7 += [
    H1('Chapitre 7 : Sprint 4 — Suivi mobile des livraisons, GPS temps réel, transit inter-dépôts et réclamations client'),
    H2('1. Introduction'),
...
c7.append(make_table(
    ...
    [
        ...
        ['US4', 'Livreur', 'Mettre à jour le statut (LIVRE, REPORTE, RETOUR, DEPOT, TENTATIVE) + COD', '/api/livreur/orders/{piece}/status + /encaisser', 'Melek', '4 j'],
        ['US5', 'Livreur', 'Démarrer livraison active : diffuser position GPS en temps réel via SignalR', '/start-heading + /location/ping + /ping-batch', 'Tawfik', '5 j'],
        ['US6', 'Livreur', 'Optimiser la tournée : voisin le plus proche + Haversine + ETA via OSRM', '/api/livreur/tournee/optimize', 'Melek', '4 j'],
        ['US7', 'Livreur transit', 'Gérer les missions de transit inter-dépôts par scan de codes-barres', '/api/transit/my-missions/* + mobile_scanner', 'Tawfik', '5 j'],
        ...
        ['US9', 'Client', 'Suivre la commande en temps réel : TrackingStateCard (4 états adaptatifs)', '/api/tracking-state + SignalR LocationUpdate', 'Tawfik', '4 j'],
        ['US10', 'Client', 'Déposer une réclamation avec photos (HEIC/HEIF), suivre et chatter via SignalR', '/api/reclamations/* + SignalR NouveauMessage', 'Melek', '4 j'],
        ['US11', 'Admin', 'Superviser les livreurs actifs, leurs zones et leurs alertes (heatmap)', '/api/admin/livreurs, /api/livreur/map/heatmap', 'Tawfik', '3 j'],
    ],
))
...
c7 += [
    ...
    N("Lorsque le livreur démarre une livraison active (POST /api/livreur/orders/{piece}/start-heading), la Web API enregistre le début de diffusion. Le service LivreurLocationService (plugin Geolocator, Flutter) capture la position GPS toutes les 10 secondes et l'envoie via POST /api/livreur/location/ping. La Web API effectue un UPSERT dans F_LIVREUR_POSITION (latitude, longitude, timestamp) et F_LIVREUR_POSITION_HISTORY pour l'audit, puis diffuse les coordonnées au client concerné via l'événement SignalR LocationUpdate (groupe client-{userId}). Côté client Flutter, la carte live LiveDeliveryMapSheet reçoit ces coordonnées et repositionne le marqueur du livreur en temps réel. En cas de perte de réseau, les positions sont stockées dans une file d'attente locale et envoyées en lot via POST /api/livreur/location/ping-batch à la reconnexion."),
    ...
    N("Le livreur de transit ouvre la mission dans transit_mission_details_screen et active le scanner via transit_barcode_scanner_screen (plugin mobile_scanner avec retour sonore). Chaque scan envoie le code-barres à la Web API (PUT /api/transit/my-missions/{id}/scan). La Web API vérifie que l'article appartient à la mission, marque la ligne correspondante comme reçue dans F_TRANSFERTS et retourne une confirmation. La mission est clôturée lorsque tous les articles ont été validés."),
    ...
    N("La carte TrackingStateCard interroge GET /api/tracking-state toutes les 15 secondes. Le backend retourne l'état courant parmi quatre valeurs : AT_DEPOT (commande en dépôt, aucun livreur assigné), IN_DELIVERY_QUEUE (BL créé, livreur assigné mais pas encore en route), HEADING_TO_YOU (livreur en route, F_LIVREUR_POSITION disponible), TERMINAL (livré, retourné ou annulé). En état HEADING_TO_YOU, la carte affiche la position GPS du livreur mise à jour via SignalR LocationUpdate, l'ETA en minutes, la distance restante et un indicateur de fraîcheur (vert < 30s, orange < 2 min, rouge sinon). Le client peut appeler ou envoyer un SMS au livreur directement depuis cet écran."),
    ...
    N("L'écran livreur_stats_screen affiche les indicateurs de performance : livraisons du jour, taux de réussite, montant total encaissé, tendance des 7 derniers jours (sparkline), répartition par statut. Le livreur peut soumettre sa caisse au dépôt (POST /api/livreur/cashbox/remettre) et consulter l'historique de ses remises."),
    ...
    N("La carte adaptative TrackingStateCard affiche l'état de la commande selon les données de /api/tracking-state. L'état AT_DEPOT affiche un message informatif. L'état IN_DELIVERY_QUEUE affiche le nom du livreur et un bouton d'appel. L'état HEADING_TO_YOU affiche la carte live avec la position GPS, l'ETA, la distance restante et l'indicateur de fraîcheur (vert, orange, rouge). L'état TERMINAL affiche le message final. Le client peut appeler ou envoyer un SMS au livreur directement."),
    ...
    N("L'interface React permet au client de déposer une réclamation (POST /api/reclamations), de sélectionner le motif (COLIS_ENDOMMAGE, NON_LIVRE, MAUVAIS_ARTICLE, etc.), de joindre des photos et de suivre le traitement. L'interface Flutter (client_create_claim_screen, client_claim_details_screen) offre les mêmes fonctionnalités depuis le mobile avec support des formats photo HEIC et HEIF (iPhone). Le chat intégré permet d'échanger des messages avec le confirmateur en temps réel via SignalR (NouveauMessage)."),
...
c8 += [
    ...
    H2('1. Introduction'),
    N("Ce chapitre présente le cinquième et dernier sprint du projet. À ce stade, l'ensemble des fonctionnalités métier a été développé : parcours client, backoffice, confirmation des commandes, réclamations, livraisons mobiles et transit. Ce sprint concentre les efforts sur la stabilisation de la solution, la construction des tableaux de bord avancés, l'intégration du chatbot administrateur basé sur n8n, la génération des exports PDF et Excel, le renforcement de la sécurité et la préparation des livrables finaux."),
    N("Ce sprint marque également la finalisation des fonctionnalités de suivi client avancées et la mise en place des notifications automatiques par push mobile pour les événements critiques (changement de statut, nouvelle réclamation, livraison imminente)."),
...
c8.append(make_table(
    ...
    [
        ...
        ['US9', 'Tous', 'Documentation technique et préparation des livrables de soutenance', 'README, Swagger, rapport PFE', 'Melek/Tawfik', '3 j'],
    ],
))
...
c8 += [
    ...
    N("L'écran livreur_stats_screen présente les indicateurs de performance individuels sur trois périodes (jour, semaine, mois) : nombre de livraisons effectuées, taux de réussite, montant total encaissé, répartition par statut, tendance 7 jours (sparkline) et rang parmi les livreurs actifs. Le livreur peut également soumettre sa caisse quotidienne au dépôt directement depuis cet écran."),
...
c8.append(make_table(
    ...
    [
        ...
        ['Scénario de démonstration', 'Valider l\'intégration bout en bout de la solution.', 'Le parcours complet fonctionne sans erreur.', 'À valider'],
    ],
))
c8 += [
    H2('7. Conclusion'),
    N("Ce dernier sprint a permis de stabiliser la solution, de renforcer la qualité globale et de préparer les livrables finaux. Le tableau de bord avancé fournit à l'administrateur une vision complète et synthétique de l'activité. Le chatbot n8n apporte une interface conversationnelle pour interroger les données métier. Les exports PDF et Excel facilitent l'exploitation des données. Les notifications automatiques renforcent la réactivité des utilisateurs aux événements critiques. L'ensemble de la solution est désormais stable, testée et prête pour la démonstration finale."),
    BR(),
]

# ─── Insertion dans l'ordre : c6, c7, c8 avant la Conclusion générale ────────
all_new = c6 + c7 + c8
ins(anchor, all_new)
